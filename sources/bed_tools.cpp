...
bed_entry::bed_entry ( std::string chrom, int start, int stop, std::string name, int score, char strand)
{
    if(start > stop) {
        ...
        throw std::logic_error("Error : start > stop");
    }
    m_chrom = chrom;
    m_start = start;
    m_stop = stop;
...
std::string bed_entry::getStringEntry(char sep) const {
    return m_chrom + sep + std::to_string(m_start) + sep + std::to_string(m_stop) + sep + m_name + sep + std::to_string(m_score) + sep + m_strand;
}
...
bed_entry bed::readBedLine(int count) {
    ...
    if (col == 3) {
        ...
        std::cout << "incorrect line, returning empty entry\r";
        return bed_entry();
    } else {
        start = stoi(tstart);
        stop = stoi(tstop);
...
void bed::writeFullBed() {
    std::cout << "Saving full file" << std::endl;
    for(const auto& entry: m_content) {
        writeBedLine(entry);
    }
}

void bed::setupAndWriteBed(std::string filename) {
    m_type = write;
    m_output = std::ofstream(filename);
    writeFullBed();
}
...
void bed::updateTags() {
    for(int i(0); i < m_content.size(); i++) {
        m_tags[m_content[i].getName()] = i;
    }
}

bed_entry bed::getBedByTag(std::string tag)
{
    if(m_tags.empty()) {
        updateTags();
    }
    return m_content[m_tags[tag]];
}
...
std::vector< std::string > sorted_bed::getChroms() const
{
...
std::map <bed_entry, std::vector <bed_entry>> sorted_bed::getOverlap (sorted_bed& toOverlap) {
    ...
    for(const auto &chrom: getChroms()) {
        ...
        std::cout << "Overlapping on chrom " << chrom << " with " << currentChrom.size() << " overlaps.\r";
...
sorted_bed::sorted_bed(std::string filename, int skip) {
    m_input = std::ifstream(filename);
    int index(0), count(0);
    if(skip > 0) {
        std::string junk;
        for(int i(0); i < skip; i++) {
            std::getline(m_input, junk);
        }
    }
    while(!m_input.eof()) {
...
std::vector <bed_entry> sorted_bed::getBedByID(std::string id) const {
    std::vector <bed_entry> output;
    for (const auto &pair : m_indexes.at(id)) {
...
std::map <int, int> AOEbed::countVector(std::vector <AOE_entry> input) {
    ...
    for(const AOE_entry& entry: input) {
        ...
        while(pos < entry.getStop()) {
            result[entry.getRelativePos(pos)] ++;
            pos ++;
        }
    }
    return result;
